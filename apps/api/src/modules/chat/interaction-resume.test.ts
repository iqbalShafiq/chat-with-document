...
const f = vi.hoisted(() => ({ store: {} as Record<string, ReturnType<typeof vi.fn>>, resolve: vi.fn(), resolveSession: vi.fn(), releaseRecipe: vi.fn(), acquire: vi.fn(), releaseRun: vi.fn(), enqueueStart: vi.fn(), enqueueResume: vi.fn(), interactionGet: vi.fn(), interactionClaim: vi.fn(), interactionRelease: vi.fn(), interactionConsume: vi.fn(), queueAdd: vi.fn(), strip: vi.fn((message: unknown) => message), touch: vi.fn(), title: vi.fn(), enqueueTitle: vi.fn(), normalizeTitle: vi.fn() }));
...
describe("chat route side-effect state machines", () => {
  beforeEach(() => { vi.clearAllMocks(); f.store = { getMeta: vi.fn(async () => ({ userId: USER_ID, sessionId: SESSION_ID, modelId: metadata.modelId, reasoningEffort: metadata.reasoningEffort })), status: vi.fn(async () => ({ status: "running", lastEventId: 0 })), openWithMeta: vi.fn(), close: vi.fn().mockResolvedValue(undefined), setStopFlag: vi.fn() }; f.resolve.mockResolvedValue(recipe); f.acquire.mockResolvedValue(true); f.enqueueStart.mockResolvedValue({ accepted: true }); f.enqueueResume.mockResolvedValue({}); f.interactionGet.mockResolvedValue(interaction); f.interactionClaim.mockResolvedValue({ token: "claim-token-1" }); f.interactionRelease.mockResolvedValue(undefined); f.interactionConsume.mockResolvedValue({ state: "consumed" }); f.queueAdd.mockResolvedValue(undefined); f.releaseRecipe.mockResolvedValue(undefined); f.releaseRun.mockResolvedValue(undefined); f.touch.mockResolvedValue(undefined); f.title.mockResolvedValue(undefined); f.enqueueTitle.mockResolvedValue(undefined); f.normalizeTitle.mockImplementation((raw: string) => { const collapsed = raw.replace(/\s+/g, " ").trim(); return collapsed.length > 0 ? Array.from(collapsed).slice(0, 48).join("") : null; }); f.resolveSession.mockResolvedValue({ id: SESSION_ID, userId: USER_ID, projectId: null, title: null }); });
  it("fresh valid request ensures the session, resolves, opens, enqueues once, and subscribes", async () => { const response = await app.request("/api/chat", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(messageBody()) }); expect(response.status).toBe(200); expect(f.resolveSession).toHaveBeenCalledWith({ userId: USER_ID, sessionId: SESSION_ID }); expect(f.resolve).toHaveBeenCalledOnce(); expect(f.acquire).toHaveBeenCalledOnce(); expect(f.store.openWithMeta).toHaveBeenCalledOnce(); expect(f.enqueueStart).toHaveBeenCalledOnce(); expect(f.releaseRecipe).not.toHaveBeenCalled(); expect(f.title).toHaveBeenCalledWith({ userId: USER_ID, sessionId: SESSION_ID, title: "hello" }); expect(f.enqueueTitle).toHaveBeenCalledWith({ sessionId: SESSION_ID, userId: USER_ID, seed: "hello", prompt: "hello" }); });
  it("caps the enqueued prompt while the seed stays the normalized snippet", async () => {
    const long = "x".repeat(2500);
    const response = await app.request("/api/chat", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ type: "messages", messages: [{ role: "user", content: long }], metadata }) });
    expect(response.status).toBe(200);
    const payload = f.enqueueTitle.mock.calls.at(-1)?.[0] as { seed: string; prompt: string };
    expect(payload.seed).toBe("x".repeat(48));
    expect(payload.prompt).toBe("x".repeat(2000));
  });
  ...
  it("does not resolve the recipe when the session cannot be created", async () => { f.resolveSession.mockRejectedValueOnce(new Error("session")); const response = await app.request("/api/chat", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(messageBody()) }); expect(response.status).toBe(404); expect(f.resolve).not.toHaveBeenCalled(); expect(f.enqueueStart).not.toHaveBeenCalled(); });
