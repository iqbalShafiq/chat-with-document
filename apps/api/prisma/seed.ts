...
const MODELS = [
  ...
  },
  {
    ...
    supportsReasoningEfforts: false,
  },
];
...
async function main() {
  ...
  for (const provider of PROVIDERS) {
    const row = await prisma.modelProvider.upsert({
      ...
      update: { name: provider.name, sortOrder: provider.sortOrder },
      create: provider,
    });
    providers.set(provider.slug, row);
  }
  ...
  for (const model of MODELS) {
    const { modelId, providerSlug, supportsReasoningEfforts, ...rest } = model;
    const provider = providers.get(providerSlug);
    if (!provider) throw new Error(`unknown provider slug: ${providerSlug}`);
    const row = await prisma.chatModel.upsert({
      where: { modelId },
      update: { ...rest, providerId: provider.id },
      create: { ...rest, modelId, providerId: provider.id },
    });
    // OpenAI-style effort levels only for models that support them; clean up
    // junction rows for models that no longer support efforts.
    if (supportsReasoningEfforts) {
      ...
      await prisma.modelReasoningEffort.deleteMany({ where: { modelId: row.id } });
    }
  }
  ...
  console.log(
    `[seed] providers=${PROVIDERS.length} models=${MODELS.length} efforts=${efforts.length} deactivated=${deactivated.count}`,
  );
}
