        ...
        <div className="mt-1.5 flex flex-col gap-2.5">
          ...
          {previewUrl ? (
            <button
              ...
              className="flex w-full cursor-pointer items-center gap-2 rounded-lg border border-white/10 bg-white/[0.03] px-2.5 py-2 text-left transition hover:bg-white/[0.07] active:scale-[0.99] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent-ring"
            >
              ...
              <span className="shrink-0 text-[10px] text-text-faint">v{build.version}</span>
            </button>
          ) : (
          ...
          {versions.length > 1 ? (
            <ol aria-label="Versi" className="flex flex-col gap-1 border-t border-white/10 pt-2">
              {versions.map((entry) => (
                <li key={entry.version} className="flex items-center gap-2 text-[11px] text-text-muted">
                  <span>
                    v{entry.version}
                    {entry.stable ? " (stabil)" : null}
                  </span>
                  {!entry.stable && entry.status === "ready" ? (
                    <Button size="sm" variant="secondary" className="ml-auto" onClick={() => onRollback(build.siteId, entry.version)}>Rollback</Button>
                  ) : null}
                </li>
              ))}
            </ol>
