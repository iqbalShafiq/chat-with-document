...
import { listScopeSites, listTasks } from "#/lib/api-artifacts";
...
export function ChatSidebar({
  ...
  // Badge counts for the session-scoped artifact entries.
  const refreshArtifactCounts = useCallback(async () => {
    if (!activeSessionId) {
    ...
    try {
      const tasks = await listTasks(activeSessionId);
      setOpenTaskCount(tasks.filter((t) => t.status !== "done").length);
    } catch {
      setOpenTaskCount(null);
    }
    try {
      const sites = await listScopeSites(activeSessionId);
        ...
        <div className="px-2 pb-2">
          ...
          >
            ...
            {openTaskCount !== null && openTaskCount > 0 ? (
              <CountBadge count={openTaskCount} label="open tasks" />
            ) : null}
          </button>
          ...
          >
            ...
            <span className="min-w-0 flex-1 truncate font-medium">Sites</span>
