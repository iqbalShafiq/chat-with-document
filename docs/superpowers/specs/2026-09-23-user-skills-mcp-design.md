...
- `UserMcpServer`: `id, userId, name (unique per user), url (https only), authType (none|bearer), credentialsRef (envelope AES-256-GCM at-rest, hanya server), headersRef (envelope terenkripsi untuk custom headers [{name, value}]), allowedToolsJson (subset review), isEnabled (global default), status (untested|ok|error), lastCheckedAt?, lastError?, createdAt, updatedAt`. Cap: ≤5 per user. Custom headers: ≤16, nama token ≤128 (selain `authorization`), value ≤2048. v1 transport HANYA `streamableHttp`; `stdio` dilarang (server tidak spawn command user). OAuth penuh deferred (§5).
...
- `POST /api/mcp-servers/test` (tanpa persist): body `{ url, authType, token?, headers?, serverId? }` → `McpClient` + timeout ±15s → `connect()` → kembalikan `{ ok, toolCount, tools: [{name, description}] }` atau `{ ok:false, error }` yang bounded (tanpa secret/stack). `close()` selalu. `serverId` milik sendiri membuat test memakai secret tersimpan saat field kosong. dipakai tombol Test di modal maupun sebelum save.
...
*Assumptions (koreksi bila salah): v1 single-MD skill; MCP auth none/bearer + custom headers; credential envelope AES-256-GCM via `MCP_CREDENTIALS_KEY` (prod wajib, dev ephemeral); caps 20 skills / 5 MCP; localStorage untuk seleksi terakhir; implementasi di branch baru.*
