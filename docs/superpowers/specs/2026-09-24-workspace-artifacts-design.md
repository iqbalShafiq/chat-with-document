...
- `create_pdf_report(input: {title, markdown, assetIds[], citationMap?, projectId?})`: resolve asset IDs dalam scope → render pdfkit (SVG chart via svg-to-pdfkit; PNG/JPEG via doc.image) → simpan derived `Document(kind=report)` dengan `reportSource { markdown, svgAssets?, imageIds? }` (sumber beku, agar edit title-only tidak menghapus isi) → kembalikan `documentId`. `edit_pdf_report(id, ...)` = revisi di file yang sama; bagian yang tidak dikirim memakai reportSource tersimpan. `GET /api/reports/:id/pdf` menyajikan bytes (scoped) untuk preview in-chat.
...
- Schedule worker `workspace-schedule` (reuse pola `profile-summary`/`site-build` queue): eksekusi prompt tersimpan dalam scope beku → start chat run di sesi asal (pipeline `chat-run` yang sama dengan POST /api/chat; balasan follow-up = notifikasi) → sesi sibuk retry 5 menit tanpa menghitung percobaan; gagal 3x → status `failed` (dead-letter).
